...
module.exports = {
  ...
  consumeStatusBar(statusBar) {
    ...
    // Service exchange remains synchronous, while DOM construction and
    // tooltip registration happen after every package has finished its
    // bootstrap. A service disappearing in the meantime cancels the mount.
    queueMicrotask(() => {
      if (disposed || !this.active || this.activationGeneration !== generation) return;
      ...
      observedFilesStatusTile = statusBar.addRightTile({
      ...
      this.updateStatusBarForItem(lumine.workspace.getCenter().getActivePaneItem());
    });
  ...
  consumeLinterRegistry(registerIndie) {
    log("Consuming linter.registry service");
  ...
  async compile() {
    ...
    if (editor.getFileState() !== FileState.UNMODIFIED) {
      await editor.save();
    }
  ...
  openPdf() {
    ...
    lumine.workspace
      ...
      .then(() => {
        lumine.notifications.addInfo(`Opened ${path.basename(outputPath)}`);
      })
      .catch((error) => {
  ...
  waitForBuildAndOpen(filePath, outputPath) {
    ...
    const openAfterBuild = () => {
      disposable.dispose();
      setTimeout(() => {
        lumine.workspace.open(outputPath, { searchAllPanes: true });
      }, 100);
    };
